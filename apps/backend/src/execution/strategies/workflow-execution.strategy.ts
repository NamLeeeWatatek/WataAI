    ...
    async execute(
        ...
        for (let i = 0; i < config.steps.length; i++) {
            ...
            if (step.execution.type === ExecutionType.AI_GENERATION) {
                stepResult = await this.aiStrategy.execute(step.execution as AiExecutionConfig, stepInputs, context);
            } else if (step.execution.type === ExecutionType.HTTP_WEBHOOK) {
                stepResult = await this.httpStrategy.execute(step.execution as HttpExecutionConfig, stepInputs, context);
            } else {
                throw new Error(`Unsupported execution type in workflow step: ${(step.execution as any).type}`);
            }
            ...
            if (context?.jobId && context.workspaceId) {
                ...
                await this.creationJobsService.update(context.jobId, context.workspaceId, {
                    progress,
                    outputData: {
                        steps: results,
                        partial: true
        ...
        return {
            ...
            result: results[config.steps[config.steps.length - 1].id]?.result || results[config.steps[config.steps.length - 1].id],
        };
    }
}
