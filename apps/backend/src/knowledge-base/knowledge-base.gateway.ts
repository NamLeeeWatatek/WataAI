...
export class KnowledgeBaseGateway
    ...
    handleDisconnect(client: Socket) {
        this.logger.log(`Client disconnected from KnowledgeBaseGateway: ${client.id}`);
    }

    @OnEvent('kb.processing.update')
    handleProcessingUpdate(payload: any) {
        this.logger.debug(`Sending processing update via WebSocket: ${payload.documentId} - ${payload.progress}%`);
