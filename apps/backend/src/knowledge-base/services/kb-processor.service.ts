    ...
    private async handleProcessDocument(job: Job<any>) {
        ...
        try {
            ...
            if (!content && document.fileUrl) {
                ...
                this.logger.warn(`Document ${documentId} has no content but has fileUrl. Extraction should have happened before queuing.`);
            }
            ...
            this.logger.log(`📄 Document ${document.id}: Created ${chunks.length} chunks`);
            ...
            for (let i = 0; i < chunks.length; i += batchSize) {
                ...
                await job.updateProgress(Math.round(((i + batch.length) / chunks.length) * 20)); // First 20% for chunking/saving
            }
            ...
            const processingResult = await this.embeddingsService.processChunksWithProgress(
                ...
                async (processed, total) => {
                    this.processingQueue.updateJobProgress(internalJobId, processed, total);
