...
import { Plus, Trash2, ShieldCheck, Zap, Sparkles, Bot, Cloud, Cpu, Settings, Stars, RefreshCw, Grid, List } from 'lucide-react';
          ...
          {viewMode === 'grid' ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              {paginatedConfigs.map((provider) => {
                const IconComponent = getProviderIcon(provider.provider?.id || provider.providerId);
                return (
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2">
                              <Badge variant="outline" className="text-[10px] uppercase">{provider.provider?.label || provider.providerId}</Badge>
                              {provider.isVerified && <Badge variant="secondary" className="bg-green-500/10 text-green-600 border-green-200 text-[10px]"><ShieldCheck className="size-3 mr-1" /> Verified</Badge>}
                            </div>
                          </div>
                        </div>
                        <div className={cn(
                ...
                columns={[
                  {
                    ...
                    label: 'Provider Name',
                    render: (v, r) => {
                      const Icon = getProviderIcon(r.provider?.id || r.providerId);
                      return (
                        <div className="flex items-center gap-3 py-1">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">{r.provider?.label || r.providerId}</div>
                          </div>
                        </div>
                      );
                    }
                  },
                  {
                    ...
                    render: (v) => (
                      <div className={cn(
                        ...
                        v ? "bg-green-500/10 text-green-600 border-green-200" : "bg-muted text-muted-foreground border-border"
                      )}>
                        <div className={cn("size-1.5 rounded-full", v ? "bg-green-500" : "bg-muted-foreground")} />
                        {v ? 'Active' : 'Inactive'}
                      </div>
                    )
                  },
                  {
                    ...
                    render: (v) => (
                      <div className="flex flex-wrap gap-1 max-w-[300px]">
                        ...
                        {(v?.length || 0) > 3 && (
                          <Badge variant="outline" className="text-[10px]">+{v!.length - 3}</Badge>
                        )}
                      </div>
                    )
                  },
                  {
                    ...
                    render: (_, r) => (
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="sm" onClick={() => handleOpenDialog(r)}><Settings className="size-4" /></Button>
                        <Button variant="ghost" size="sm" className="text-destructive hover:bg-destructive/10" onClick={() => setDeleteId(r.id)}><Trash2 className="size-4" /></Button>
                      </div>
                    )
                  }
                ]}
