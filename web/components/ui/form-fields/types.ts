...
export type NodeProperty = Omit<ApiFormField, 'type' | 'options' | 'defaultValue'> & {
    type: string | 'string' | 'text' | 'textarea' | 'number' | 'boolean' | 'checkbox' | 'radio' | 'select' | 'multi-select' | 'slider' | 'color' | 'json' | 'file' | 'files' | 'key-value' | 'dynamic-form' | 'channel-select' | 'channel-selector' | 'canvas-editor'
    displayName?: string
    helpText?: string
    hint?: string
    rows?: number
